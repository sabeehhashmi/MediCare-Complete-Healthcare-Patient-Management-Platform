	...
	jQuery.mbYTPlayer = {
		...
		buildPlayer: function (options) {
			...
			return this.each(function () {
				...
				// Commented out mobile restriction to allow background videos on mobile
				...
				// Allow overlay on all devices including mobile
				...
				// add YT API to the header - Optimized to prevent multiple loads
				if(!ytp.YTAPIReady && !document.getElementById("YTAPI")){
					var tag = document.createElement('script');
					tag.src = "https://www.youtube.com/player_api"; // Changed to HTTPS
					tag.id = "YTAPI";
					var firstScriptTag = document.getElementsByTagName('script')[0];
					firstScriptTag.parentNode.insertBefore(tag, firstScriptTag);
				}else if(ytp.YTAPIReady){
					setTimeout(function(){
						jQuery(document).trigger("YTAPIReady");
				...
				jQuery(document).on("YTAPIReady", function () {
					...
					jQuery(document).on("getVideoInfo_" + YTPlayer.opt.id, function () {
						...
						// Modified to allow background videos on mobile devices
						if(isDevice && !YTPlayer.isBackground){
		...
		getDataFromFeed: function (videoID, YTPlayer) {
			//Get video info from FEEDS API - Optimized for faster loading

			YTPlayer.videoID = videoID;
			
			// Skip data fetching for background videos to improve performance
			if (YTPlayer.isBackground) {
				YTPlayer.opt.ratio = "16/9"; // Default ratio for background videos
				if(!YTPlayer.isInit){
