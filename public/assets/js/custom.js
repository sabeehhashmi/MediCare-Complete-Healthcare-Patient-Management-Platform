  ...
  $(function () {
    ...
    $('input[name="inOut"]').each(function () {
      ...
      $input.daterangepicker(
        ...
        function (start) {
          const formattedDayMonth = start.format("D ");
          const formattedDayYear = start.format("MMMM YYYY");
          const formattedDate = `<div class="selected-date"><h6>${formattedDayMonth}${formattedDayYear}</h6></div>`;
...
const localeEn = {
  ...
  document.querySelectorAll('.custom-file input[type="file"]').forEach(input => {
    input.addEventListener('change', function () {
      const fileNameSpan = this.closest('.custom-file')
