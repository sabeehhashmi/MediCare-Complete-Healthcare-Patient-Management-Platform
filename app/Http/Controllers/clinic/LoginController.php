...
class LoginController extends Controller
{
    ...
    public function login()
    {
        ...
        if (Auth::check() && (Auth::user()->role == CLINIC_ROLE)) {
    ...
    public function Register()
    {
        ...
        $terms = Article::where('type', 3)->first();
        // dd($terms);
        ...
        // echo Hash::make('Hello@1985');
        return view('clinic.register',compact('country_list','emirates_list','area_list','terms', 'selected_country'));
    }
        ...
        if ($validator->fails()) {
            ...
            if (!$check_exist) {
                ...
                if (!$check_exist_phone) {
                    ...
                    //Mail::to($user->email)->send(new VerifyEmail($user, 'clinic'));
...
public function verifyLoginOtp(Request $request)
{
    ...
    public function check_login_b(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            // Validate request
            if (Auth::attempt(['email' => $request->email, 'password' => $request->password, 'deleted' => 0, 'role' => CLINIC_ROLE])) {
                if (Auth::check()) {
