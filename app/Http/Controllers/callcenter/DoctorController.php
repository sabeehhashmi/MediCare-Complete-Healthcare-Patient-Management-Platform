...
class DoctorController extends Controller
{
    public function index(Request $request){
        ...
        $loginuserid = Auth::id();
        $callcenter = CallCenterUserDetail::where('user_id',$loginuserid)->get()->first();
        $callcenterId  = $callcenter->id;
        $callcenter_id = null;
        $hospital_id = $request->hospital_id;
        $clinic_id = $request->clinic_id;

        return view('callcenter.doctors.doctors',compact('page_heading','module_heading' ,'hospital_id', 'clinic_id'));
    }
    ...
    public function detail($id){
        ...
        $hospital = Hospital::where('id',$doctor->hospital_id)->get()->first();
        ...
        // dd($doctor);
        return view('callcenter.doctordetail',compact('insurences','page_heading','module_heading','doctor', 'totalappointments', 'pendingappointments', 'confirmappointments', 'completedappointments', 'cancelledappointments'));
    }
...
$time_slot = [
    ...
    return view('callcenter.doctors.patientAppointment.viewAppointment',compact('page_heading','booking_time_slot','users','time_slot','doctor_id'));
}
    ...
    public function create($id=''){
        ...
        $hospital_name = Hospital::whereHas('user',function($q) use ($id){
            $q->where('active', 1);
        })->get();
        ...
        return view('callcenter.doctors.createdoctor',compact(
    ...
    public function appointments4(Request $request,$id){
        ...
        return view('callcenter.doctors.appointments',compact('page_heading','patient','time_slot','doctor_id'));
    ...
    public function availability($id){
        ...
        $time_slot = TIME_SLOTS;
// dd($friday_time_slot);
        return view('callcenter.availability',compact('page_heading','module_heading','time_slot',
        'sunday_availability',
        'sunday_time_slot',
        'monday_availability',
        'monday_time_slot',
    ...
    public function instantAppointment($id){
        ...
        return view('callcenter.instantAppointment',compact('page_heading','module_heading',
        'doctor_id', 'doctor'));
    }
    public function temporaryUnavailable($id){
        ...
        return view('callcenter.temporaryUnavailable',compact('page_heading','module_heading','time_slot',
        'doctor_id', 'doctor'));
    }
    ...
    public function appointments(Request $request){
        ...
        return view('callcenter.appointment', compact(
            'page_heading','module_heading',
            // 'todayAppointments',
    ...
    public function holiday($id){
        ...
        $doctor_id = $id;
        $doctor = Doctor::find($doctor_id);
        return view('callcenter.holiday',compact('page_heading','module_heading','doctor_id', 'doctor'));
    }
    public function save(REQUEST $request)
    {
        ...
        $o_data['redirect'] = route('callcenter.doctors', $params);
...
DoctorDocument::where('doctor_id', $doctor->id)
        ...
        return DataTables::eloquent($users)
            ->addColumn('action', function($user) use ($request) {
                ...
                $action = '<div class="dropdown mt-4 mt-sm-0">
                    ...
                    <div class="dropdown-menu">
                        <a class="dropdown-item complete-link" href="'.route('callcenter.doctordetail',['id'=>$user->id]).'">View Profile</a>
                        <a class="dropdown-item complete-link" href="'.route('callcenter.editDoctor', $params).'">Edit Profile </a>
                        <a class="dropdown-item" data-role="unlink" data-message="Do you want to remove the doctor?  This may be linked with other sections" href="'.route('callcenter.deleteDoctor', ['id' => encrypt($user->id)]).'">
                            <i class="flaticon-delete-1"></i> Delete Doctor
                        </a>
                        <a class="dropdown-item complete-link" href="'.route('callcenter.appointments',['doctor_id'=>$user->id]).'">View Appointments</a>
                        <a class="dropdown-item complete-link" href="'.route('callcenter.availability',['id'=>$user->id]).'">Schedule Appointment Slots</a>
                        <a class="dropdown-item complete-link" href="'.route('callcenter.temporaryunavailable',['id'=>$user->id]).'">Marks Temporary Unavailability </a>
                        <a class="dropdown-item complete-link" href="'.route('callcenter.holiday',['id'=>$user->id]).'">Mark Holiday Date </a>
                        <a class="dropdown-item complete-link" href="'.route('callcenter.instantappointment',['id'=>$user->id]).'">Mark Instant Appointment Date</a>
                    </div>
                </div>';
                return $action;
            })
            ...
            ->addColumn('phone_number', function($item) {
                return ($item->user->phone)?'+'.$item->user->dial_code.$item->user->phone:'';
            })
            ->addColumn('doctor_qualifications', function($user) {
                return $user->qualifications ? $user->qualifications->pluck('title')->implode(', ') : 'N/A';
            })
    ...
    public function instantAppointmentSave(Request $request) {
        ...
        try {
            ...
            return response()->json([
                ...
                'oData' => [
                    'redirect' => route('callcenter.doctors')
                ]
            ]);
        } catch (Exception $e) {
            DB::rollback();
    ...
    public function holiday_delete($id) {
        $status = "0";
        $message = "";
        $o_data['redirect'] = route('callcenter.doctors');
