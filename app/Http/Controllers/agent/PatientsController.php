...
//            ->get();
        ...
        return view('agent.patients.index', compact('page_heading'));
    }
    ...
    public function members($id)
    {
        ...
        return view('agent.patients.members', compact('page_heading', 'records', 'patient_id'));
    }
    ...
    public function appointments($id){
        ...
        ->get();
        return view('agent.patients.appointment',compact('page_heading','patient','patient_id'));
    ...
    public function create($id = '')
    {
        ...
        return view('agent.patients.create', compact(
            'page_heading',
            'language_spoken',
            'insurence_list',
            'sub_insurence_list',
    ...
    public function create_appointment($patient_id, $id = '')
    {
        ...
        if ($id) {
            ...
            if ($patient_id != $row->user_id) {
                return redirect()->route('agent.patients.create_appointment', $patient_id);
            }
        }
        ...
        return view('agent.patients.make_appointment', compact(
            'page_heading',
            'id',
            'patients',
            'patient_id',
    ...
    public function createMember($patient_id, $id = '')
    {
        ...
        if ($id) {
            ...
            if ($patient_id != $row->user_id) {
                return redirect()->route('agent.patients.createMember', $patient_id);
            }
        }


        return view('agent.patients.create_member', compact(
            'page_heading',
            'language_spoken',
            'insurence_list',
            'patient',
    ...
    public function save(Request $request)
    {
        ...
        $o_data['redirect'] = route('agent.patients.index');
    ...
    public function saveMember(Request $request)
    {
        ...
        $o_data['redirect'] = route('agent.patients.members', $request->patient);
    ...
    public function saveAppointment(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $status = "1";
            $o_data['redirect'] = route('agent.patients.appointments', $request->patient);
        }
    ...
    public function load_data(Request $request)
{
    ...
    */
    ...
    if ($request->has('search') && ($request->search['filters'] ?? null)) {
        ...
        if ($filters['booking_from'] ?? null) {
            $date = \Carbon\Carbon::createFromFormat('d-m-Y', $filters['booking_from'])
                ->format('Y-m-d');
            $query->whereDate('created_at', '>=', $date);
        }

        if ($filters['booking_to'] ?? null) {
            $date = \Carbon\Carbon::createFromFormat('d-m-Y', $filters['booking_to'])
                ->endOfDay()
                ->format('Y-m-d H:i:s');
    ...
    */
    ...
    return DataTables::eloquent($users)
        ...
        /*
        |--------------------------------------------------------------------------
        | Global Search (Fix Phone Search Here)
        |--------------------------------------------------------------------------
        */
    ...
    if ($search = $request->get('search')['value'] ?? null) {
        ...
        $query->where(function ($q) use ($search) {
            $q->where('name', 'like', "%{$search}%")
              ...
              ->orWhereRaw("CONCAT(dial_code, phone) LIKE ?", ["%{$search}%"]);
        });
    }
})
        ...
        /*
        |--------------------------------------------------------------------------
        | Serial Number (Correct Pagination Based)
        |--------------------------------------------------------------------------
        */
        ...
        */
        ...
        ->addColumn('whatsapp_number', function ($item) {
            return $item->whatsap_dial_code
                ? '+' . $item->whatsap_dial_code . $item->whatsap_phone
                : '';
        })
        ...
        */
        ->addColumn('action', function ($user) {
            ...
            $action .= '<a class="dropdown-item"
                            href="' . route('agent.patients.edit', ['id' => $user->id]) . '">
                            Edit
                        </a>';

            $action .= '<a class="dropdown-item"
                            href="'. route('agent.patients.members',['id'=>$user->id]) . '">
                            Members
                        </a>';

            $action .= '<a class="dropdown-item"
                            href="' . route('agent.hospitalAppointments',['patient_id'=>$user->id]) . '">
                            Appointments
                        </a>';

            $action .= '<a class="dropdown-item"
                            data-role="unlink"
                            data-message="Do you want to remove the patient?"
                            href="' . route('agent.patients.delete', ['id' => encrypt($user->id)]) . '">
                            Delete
                        </a>';
    ...
    //                 $action .= '<a class="dropdown-item complete-link" href="'.route('agent.patients.edit_appointment', ['patient_id' => $user->user_id ?? null, 'id' => $user->id]).'">Edit</a>';
