...
class HospitalAppointmenstsController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        return view('agent.totalappointments', compact(
    ...
    public function appointmentLoadData(REQUEST $request){
        ...
        // ->where('doctor_patient_appointments.agent_id','=',$agent->id)
        ...
        ->addColumn('action', function($user) {
             $action = '<div class="dropdown mt-4 mt-sm-0">
                ...
                if((strtolower($user->booking_status) === 'cancelled') || (strtolower($user->booking_status) === 'completed')){
                    $action .= '<a class="dropdown-item complete-link" href="'.route('agent.appointmentdetail', ['id' => $user->id]).'">View Appointment</a>';
            }else{
                    $action .= '<a class="dropdown-item complete-link" href="'.route('agent.appointmentdetail', ['id' => $user->id]).'">View Appointment</a>';
    ...
    //                 $action .= '<a class="dropdown-item complete-link" href="'.route('agent.edit_appointment', ['id' => $user->id]).'">Edit</a>';
    ...
    public function create_appointment($id = '')
    {
        ...
        // dd($row->toArray());
        return view('agent.createAppointment', compact(
            'page_heading', 'module_heading',
            'id',
            'patients',
            'hospital_id',
    ...
    public function saveAppointment(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $status = "1";
            $o_data['redirect'] = route('agent.hospitalAppointments');
        }
    ...
    public function saveDrAppointment(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $status = "1";
            $o_data['redirect'] = route('agent.appointments', $doctor->doctor_id);
        }
    ...
    public function reports(Request $request)
    {
        ...
        $appointments = $appointmentsQuery->where($whereClauses)->orderBy('id', 'desc')->paginate(10);
        return view('agent.reports', compact('page_heading','doctors','departments','appointments', 'hospitals', 'module_heading'));
    }
    ...
    public function booking_details($id){
        ...
        return view('agent.appointment_detail',compact('id','appointment', 'module_heading', 'doctor',
...
public function doctorsByDepartment(Request $request)
{
    ...
    ->first();
        ...
        return view('agent.history',compact('id','appointments', 'module_heading',
