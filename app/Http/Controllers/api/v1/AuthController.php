...
class AuthController extends Controller
{
    ...
    public function confirm_email_code(REQUEST $request){
        $status  = "0";
        $message = "";
        $o_data  = [];
        $errors  = [];
    ...
    public function email_login_old(Request $request)
    {
        $rules = [
            'password' => 'required',
        ...
        $user = User::whereRaw("LOWER(email) = '$lemail'")->where('deleted', 0)->where(['role' => USER_ROLE])->first();
        if ($user != null) {
        ...
        if ($user = User::where('email', $request->email)->where("deleted", 0)->where(function ($query) {
            ...
            else
            {
                return response()->json([
                    'status' => "0",
                    'error' => (object) array(),
                    'message' => "The email address you entered is not  associated with social account. Kindly use email login option.",
                ], 201);
            }
            ...
            if ($request->phone && User::where('phone', ltrim($request->phone,0))->where('dial_code', $request->dial_code)->where('deleted', 0)->first() != null) {
                return response()->json([
                    'status' => "0",
                    'error' => (object) array(),
            ...
            if(!$TempUser){
                $TempUser = TempUsers::where('phone', $request->phone)->where('dial_code', $request->dial_code)->first();
            }
            $TempUser = $TempUser ? $TempUser : new TempUsers();
            $ttp= generate_otp();;
            $TempUser->fill([
                ...
                'phone_otp' => (string)$ttp,
                'email_otp' => (string)$ttp,
                'gender' =>  $gender,
                'whatsap_dial_code' => $whatsap_dial_code,
                'whatsap_phone' => $whatsap_number,
                'insurence_id' => $sub_inusurence_id,
                'sub_insurence_id' => 1,
            ...
            $otp = $TempUser->phone_otp;
            $name = $TempUser->first_name . ' ' . $TempUser->last_name;
            // $mailbody = view('email_templates.verify_mail', compact('otp', 'name'));
            
            $mobile = $request->dial_code . ltrim($request->phone,0);
            $messagen = "Thank you for using Mednero. Your OTP is ".$TempUser->user_email_otp." and valid for 10 minutes.";
            send_normal_SMS($mobile,$messagen);
            ...
            $TempUser->id = (string) $TempUser->id;
            return response()->json([
                'status' => "1",
                ...
                'user' => convert_all_elements_to_string($TempUser->toArray()),
                // 'access_token' => $token,
            ], 201);
        }
        else
    ...
    public function verify_signup_otp_social_login(REQUEST $request){
        $status  = "0";
        $message = "";
        $o_data  = [];
        $errors  = [];
    ...
    public function signup(Request $request)
    {
        ...
        $lemail = strtolower($request->email);
        User::whereRaw("LOWER(email) = '$lemail'")->where('email_verified', 0)->where('phone_verified', 0)->where(['role' => 2])->delete();
        User::where('phone', $request->phone)->where('dial_code', $request->dial_code)->where('email_verified', 0)->where('phone_verified', 0)->where(['role' => 2])->delete();
        ...
        if (User::where('phone', ltrim($request->phone,0))->where('dial_code', $request->dial_code)->where('deleted', 0)->first() != null) {
            return response()->json([
                'status' => "0",
                'error' => (object) array(),
        ...
        if(!$TempUser){
            $TempUser = TempUser::where('phone', ltrim($request->phone,0))->where('dial_code', $request->dial_code)->first();
        }
        ...
        $mobile = $request->dial_code . ltrim($request->phone,0);
        $messagen = "OTP to confirm The Le concierge registration is " . $TempUser->user_phone_otp;
    ...
    public function resend_code(Request $request)
    {
        ...
        $user->user_email_otp = (string)generate_otp();//rand(1000, 9999);
        $user->user_phone_otp=$user->user_email_otp;
        ...
        if (config('global.server_mode') == 'local') {
            \Artisan::call("app:send-login-otp-mail ".$user->id);
        } else {
            exec("php " . base_path() . "/artisan app:send-login-otp-mail " . $user->id . " temp > /dev/null 2>&1 & ");
    ...
    public function confirm_code_old(Request $request)
    {
    ...
    public function resend_phone_code(Request $request)
    {
        ...
        $messagen = "OTP to confirm Le concierge registration is " . $otp;
    ...
    public function confirm_phone_code_old(Request $request)
    {
        $rules = [
            'user_id' => 'required',
            'otp' => 'required',
...
public function user_id_login(REQUEST $request){
    $status  = "0";
    $message = "";
    $o_data  = [];
    $errors  = [];
    ...
    if ($validator->fails()) {
        ...
        $user = User::where(['id'=>$request->user_id,'role'=>USER_ROLE])->get();
        if($user->count() > 0){
            $user =$user->first();
