...
//            ->get();
        ...
        return view('admin.patients.index', compact('page_heading'));
    }
    
    public function points_index($id)
    {
        if (!get_user_permission('patients', 'r')) {
            return redirect()->route('admin.restricted_page');
        }
...
//            ->get();
        ...
        return view('admin.patients.point_index', compact('page_heading','id','user'));
    }
    ...
    public function members($id)
    {
        ...
        return view('admin.patients.members', compact('page_heading', 'records', 'patient_id'));
    }
    ...
    public function appointments($id){
        ...
        ->get();
        return view('admin.patients.appointment',compact('page_heading','patient','patient_id'));
    ...
    public function create_appointment($patient_id, $id = '')
    {
        ...
        if ($id) {
            ...
            if ($patient_id != $row->user_id) {
                return redirect()->route('admin.patients.create_appointment', $patient_id);
            }
        }
        ...
        return view('admin.patients.make_appointment', compact(
            'page_heading',
            'id',
            'patients',
            'patient_id',
    ...
    public function createMember($patient_id, $id = '')
    {
        ...
        if ($id) {
            ...
            if ($patient_id != $row->user_id) {
                return redirect()->route('admin.patients.createMember', $patient_id);
            }
        }


        return view('admin.patients.create_member', compact(
            'page_heading',
            'language_spoken',
            'insurence_list',
            'patient',
    ...
    public function save(Request $request)
    {
        ...
        $o_data['redirect'] = route('admin.patients.index');
       ...
       // dd($request->all());
        ...
        $rules = [
            ...
            'identification_document' => 'nullable|file|mimes:jpeg,png,jpg,pdf|max:5120', // 5MB max
        ];
        ...
        try {
            ...
            if ($id) {
                ...
                if (empty($user->patient_id)) {
                    $user->patient_id = \App\Helpers\PatientIdHelper::generatePatientIdByDate($user->created_at);
                }
            } else {
                $user = new User();
                $user->role = USER_ROLE;
            ...
            if ($request->hasfile('identification_document')) {
                ...
                $file_name = 'id_' . time() . uniqid() . "." . $file->getClientOriginalExtension();
                $file->storeAs(config('global.user_documents_dir', 'documents'), $file_name, config('global.upload_bucket'));
                $user->identification_document = $file_name;
            }
    ...
    public function saveMember(Request $request)
    {
        ...
        $o_data['redirect'] = route('admin.patients.members', $request->patient);
    ...
    public function saveAppointment(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $status = "1";
            $o_data['redirect'] = route('admin.patients.appointments', $request->patient);
        }
    ...
    {
        $query = User::query()->where('role', USER_ROLE)->where('deleted', 0);
        ...
        if ($request->has('search') && ($request->search['filters'] ?? null)) {
            ...
            if (isset($filters['patient_id']) && !empty($filters['patient_id'])) {
                $query->where('patient_id', 'ILIKE', "%{$filters['patient_id']}%");
            }
        ...
        $users = $query->select(['id','patient_id', 'name', 'dial_code', 'phone', 'email', 'whatsap_dial_code', 'gender', 'whatsap_phone', 'active', 'first_name', 'last_name'])
            ->orderBy('id', 'desc');
...
//                $action .= '<a class="dropdown-item complete-link" href="' . route('admin.patients.show', ['id' => $user->id]) . '">View </a>';
                $action .= '<a class="dropdown-item complete-link" href="' . route('admin.patients.edit', ['id' => $user->id]) . '">Edit </a>';
                $action .= '<a class="dropdown-item complete-link" href="'. route('admin.patients.members',['id'=>$user->id]) . '">Members</a>';
                $action .= '<a class="dropdown-item complete-link" href="' . route('admin.appointments.index',['patient_id'=>$user->id]) . '">Appointments</a>';
                $action .= '<a class="dropdown-item complete-link" href="' . route('admin.appointment_history',['id'=>$user->id]) . '">Past medical summery</a>';
                $action .= '<a class="dropdown-item complete-link" href="' . route('admin.points_index.index',['id'=>$user->id]) . '">Loyalty & Rewards </a>';
                if (get_user_permission('patients', 'd')) {
                    $action .= '<a class="dropdown-item" data-role="unlink"
                                            ...
                                            href="' . route('admin.patients.delete', ['id' => encrypt($user->id)]) . '">
            ...
            ->addColumn('sl_no', function ($user) {
             ...
             ->addColumn('patient_id', function ($user) {
                return $user->patient_id ?? 'N/A';
            })
    ...
    //             if (get_user_permission('patients', 'r')) {
    //                 $action .= '<a class="dropdown-item complete-link" href="'.route('admin.patients.edit_appointment', ['patient_id' => $user->user_id ?? null, 'id' => $user->id]).'">Edit</a>';
    ...
    public function load_point_history_data(Request $request)
{
    ...
    if ($request->has('search') && !empty($request->search['value'])) {
        ...
        $query->where(function ($q) use ($searchValue) {
            ...
            // BOOKING ID SEARCH
            $q->whereHas('appointment', function ($sub) use ($searchValue) {
            ...
            // PATIENT SEARCH
            $q->orWhereHas('appointment.user', function ($sub) use ($searchValue) {
            ...
            // CLINIC SEARCH
            $q->orWhereHas('appointment.hospital', function ($sub) use ($searchValue) {
    ...
    return DataTables::eloquent($data)
        ...
        ->addColumn('booking_id', function ($point) {

            return $point->appointment->booking_id ?? 'N/A';
        })
        ...
        ->addColumn('type', function ($point) {

            return ucfirst($point->type);
        })

        ->addColumn('booking_status', function ($point) {

            return $point->appointment->booking_status ?? 'N/A';
        })

        ->addColumn('patient', function ($point) {

            $appointment = $point->appointment;

            return $appointment->patient_member_name
                ?? $appointment->user->name
                ?? 'N/A';
        })

       
        ->addColumn('doctor', function ($point) {

            return $point->appointment->doctor->user->name ?? 'N/A';
        })

        ->addColumn('clinic', function ($point) {

            return $point->appointment->hospital->name_en ?? 'N/A';
        })

        ->addColumn('booking_date', function ($point) {

            return $point->appointment->booking_date ?? 'N/A';
        })

        ->addColumn('booking_time', function ($point) {

            return $point->appointment->booking_time_slot ?? 'N/A';
        })
         ->addColumn('consultation_fee', function ($point) {

            return $point->appointment->consultation_fee ?? 'N/A';
        })
        ->addColumn('created_at_formatted', function ($point) {

            return Carbon::parse($point->created_at)
                ->format('d-m-Y h:i A');
        })
