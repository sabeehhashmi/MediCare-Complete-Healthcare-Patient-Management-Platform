...
class PrescriptionController extends Controller
{
    ...
    public function index()
    {
        ...
        $list = Prescription::orderBy('id','desc')->get();
        $mode='list';
    ...
    public function submit(REQUEST $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $id         = $request->id;
            $check      = Prescription::whereRaw('Lower(title) = ?', [strtolower($title_en)])->where('id', '!=', $id)->get();
    ...
    public function change_status(REQUEST $request)
    {
        ...
        $item = Prescription::where(['id' => $id])->get();

        if ($item->count() > 0) {

            Prescription::where('id', '=', $id)->update(['status' => $request->status]);
            $status = "1";
    ...
    public function delete(REQUEST $request, $id)
    {
        ...
        $category_data = Prescription::where(['id' => $id])->first();
