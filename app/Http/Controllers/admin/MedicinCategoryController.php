...
class MedicinCategoryController extends Controller
{
    ...
    public function index()
    {
        ...
        $page_heading = "Medicine Category";
        $list = MedicinCategory::orderBy('id','desc')->get();
        $mode='list';
        return view('admin.medicin_categories.list', compact('mode', 'page_heading', 'list'));
    }
    ...
    public function create($id = '')
    {
        ...
        if ($id) {
            $mode = "Edit";
            $id = decrypt($id);
            $role = MedicinCategory::find($id);
            $title_en = $role->title;
            $status = $role->status;
            $title_ar = $role->title_ar;
            $title_ban = $role->title_ban;
        }
        return view('admin.medicin_categories.create', compact('mode', 'page_heading', 'id', 'status', 'title_en', 'title_ar','title_ban'));
    }
    ...
    public function submit(REQUEST $request)
    {
        ...
        $o_data     = [];
        $errors     = [];
        $o_data['redirect'] = route('admin.medicin_categories.list');
        $rules = [
            'title_en' => [
                'required',
                'min:2',
                Rule::unique('medicine_categories', 'title')
                ->ignore($request->id)
        ...
        if ($validator->fails()) {
            ...
            $id         = $request->id;
            $check      = MedicinCategory::whereRaw('Lower(title) = ?', [strtolower($title_en)])->where('id', '!=', $id)->get();

            if ($check->count() > 0) {
                $status = "0";
                $message = "Medicine Category Already Addded";
                $errors['title_en'] = 'Medicine Category Already Added';
                if($request->json_action){
                    return response()->json([
                        'success' => true,
                        'message' => "Medicine Category Already Added successfully",
                        'id' => '',
                        'name' => ''
                    ]);
                }
    ...
    public function change_status(REQUEST $request)
    {
        ...
        $item = MedicinCategory::where(['id' => $id])->get();

        if ($item->count() > 0) {

            MedicinCategory::where('id', '=', $id)->update(['status' => $request->status]);
            $status = "1";
    ...
    public function delete(REQUEST $request, $id)
    {
        ...
        $category_data = MedicinCategory::where(['id' => $id])->first();
