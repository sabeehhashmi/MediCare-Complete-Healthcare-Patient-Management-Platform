...
class ReferralController extends Controller
{
    ...
    public function index()
    {
        ...
        $list = Referral::orderBy('id','desc')->get();
        $mode='list';
        return view('admin.referrals.list', compact('mode', 'page_heading', 'list'));
    }
    ...
    public function create($id = '')
    {
        ...
        if ($id) {
            $mode = "Edit";
            $id = decrypt($id);
            $role = Referral::find($id);
            $title_en = $role->title;
            $status = $role->status;
            $title_ar = $role->title_ar;
        }
        return view('admin.referrals.create', compact('mode', 'page_heading', 'id', 'status', 'title_en', 'title_ar'));
    }
    ...
    public function submit(REQUEST $request)
    {
        ...
        $o_data     = [];
        $errors     = [];
        $o_data['redirect'] = route('admin.referrals.list');
        $rules = [
            'title_en' => [
                'required',
                'min:3',
                Rule::unique('referrals', 'title')
                ->ignore($request->id)
        ...
        if ($validator->fails()) {
            ...
            $id         = $request->id;
            $check      = Referral::whereRaw('Lower(title) = ?', [strtolower($title_en)])->where('id', '!=', $id)->get();

            if ($check->count() > 0) {
                $status = "0";
                $message = "Referrals Already Addded";
                $errors['title_en'] = 'Referrals Already Added';
                if($request->json_action){
                    return response()->json([
                        'success' => true,
                        'message' => "Referrals Already Added successfully",
                        'id' => '',
                        'name' => ''
                    ]);
                }
                ...
                if ($id) {
                    ...
                    try {
                        ...
                        DB::commit();
                        $status = "1";
                        $message = "Referrals Added Successfully";
                        if($request->json_action){
                            return response()->json([
                                'success' => true,
                                'message' => "Referrals added successfully",
    ...
    public function delete(REQUEST $request, $id)
    {
        ...
        $category_data = Referral::where(['id' => $id])->first();
