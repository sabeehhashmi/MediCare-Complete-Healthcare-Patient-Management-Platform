...
class FrequenciesController extends Controller
{
    ...
    public function index()
    {
        ...
        $page_heading = "Frequencies";
        $list = Frequency::orderBy('id','desc')->get();
        $mode='list';
        return view('admin.frequencies.list', compact('mode', 'page_heading', 'list'));
    }
    ...
    public function create($id = '')
    {
        ...
        if ($id) {
            $mode = "Edit";
            $id = decrypt($id);
            $role = Frequency::find($id);
            $title_en = $role->title;
            $status = $role->status;
            $title_ar = $role->title_ar;
            $title_ban = $role->title_ban;
        }
        return view('admin.frequencies.create', compact('mode', 'page_heading', 'id', 'status', 'title_en', 'title_ar','title_ban'));
    }
    ...
    public function submit(REQUEST $request)
    {
        ...
        $o_data     = [];
        $errors     = [];
        $o_data['redirect'] = route('admin.frequencies.list');
        $rules = [
            'title_en' => [
                'required',
                'min:3',
                Rule::unique('frequencies', 'title')
                ->ignore($request->id)
        ...
        if ($validator->fails()) {
            ...
            $id         = $request->id;
            $check      = Frequency::whereRaw('Lower(title) = ?', [strtolower($title_en)])->where('id', '!=', $id)->get();

            if ($check->count() > 0) {
                $status = "0";
                $message = "Frequencies Already Addded";
                $errors['title_en'] = 'Frequencies Already Added';
                if($request->json_action){
                    return response()->json([
                        'success' => true,
                        'message' => "Frequencies Already Added successfully",
                        'id' => '',
                        'name' => ''
                    ]);
                }
                ...
                if ($id) {
                    ...
                    try {
                        ...
                        DB::commit();
                        $status = "1";
                        $message = "Frequencies updated Successfully";
                    } catch (Exception $e) {
                        DB::rollback();
                    ...
                    try {
                        ...
                        DB::commit();
                        $status = "1";
                        $message = "Frequencies Added Successfully";
                        if($request->json_action){
                            return response()->json([
                                'success' => true,
                                'message' => "Frequencies added successfully",
    ...
    public function change_status(REQUEST $request)
    {
        ...
        $item = Frequency::where(['id' => $id])->get();

        if ($item->count() > 0) {

            Frequency::where('id', '=', $id)->update(['status' => $request->status]);
            $status = "1";
    ...
    public function delete(REQUEST $request, $id)
    {
        ...
        $category_data = Frequency::where(['id' => $id])->first();
