...
class SpecialIntrestsController extends Controller
{
    //
    public function index()
    {
        ...
        $page_heading = "Special Intrests";
        $list = SpecialIntrests::orderBy('id','desc')->get();
        $mode='list';
        return view('admin.special_intrests.list', compact('mode', 'page_heading', 'list'));
    }
    ...
    public function create($id = '')
    {
        ...
        if ($id) {
            $mode = "Edit";
            $id = decrypt($id);
            $role = SpecialIntrests::find($id);
            $title_en = $role->title;
            $status = $role->status;
            $title_ar = $role->title_ar;
        }
        return view('admin.special_intrests.create', compact('mode', 'page_heading', 'id', 'status', 'title_en', 'title_ar'));
    }
    ...
    public function submit(REQUEST $request)
    {
        ...
        $o_data     = [];
        $errors     = [];
        $o_data['redirect'] = route('admin.special_intrests.list');
        $rules = [
            'title_en' => [
                'required',
                'min:3',
                Rule::unique('special_intrests', 'title')
                ->ignore($request->id)
        ...
        if ($validator->fails()) {
            ...
            $id         = $request->id;
            $check      = SpecialIntrests::whereRaw('Lower(title) = ?', [strtolower($title_en)])->where('id', '!=', $id)->get();

            if ($check->count() > 0) {
                $status = "0";
                $message = "Special Intrests Already Addded";
                $errors['title_en'] = 'Special Intrests Already Added';
                if($request->json_action){
                    return response()->json([
                        'success' => true,
                        'message' => "Special Intrests Already Added successfully",
                        'id' => '',
                        'name' => ''
                    ]);
                }
                ...
                if ($id) {
                    ...
                    try {
                        ...
                        DB::commit();
                        $status = "1";
                        $message = "Special Intrests updated Successfully";
                    } catch (Exception $e) {
                        DB::rollback();
                    ...
                    try {
                        ...
                        DB::commit();
                        $status = "1";
                        $message = "Special Intrests Added Successfully";
                        if($request->json_action){
                            return response()->json([
                                'success' => true,
                                'message' => "Special Intrests added successfully",
    ...
    public function change_status(REQUEST $request)
    {
        ...
        $item = SpecialIntrests::where(['id' => $id])->get();

        if ($item->count() > 0) {

            SpecialIntrests::where('id', '=', $id)->update(['status' => $request->status]);
            $status = "1";
    ...
    public function delete(REQUEST $request, $id)
    {
        ...
        $category_data = SpecialIntrests::where(['id' => $id])->first();
