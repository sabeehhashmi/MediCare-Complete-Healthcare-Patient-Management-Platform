...
class DurationController extends Controller
{
    ...
    public function index()
    {
        ...
        $page_heading = "Durations";
        $list = Duration::orderBy('id','desc')->get();
        $mode='list';
        return view('admin.durations.list', compact('mode', 'page_heading', 'list'));
    }
    ...
    public function create($id = '')
    {
        ...
        if ($id) {
            $mode = "Edit";
            $id = decrypt($id);
            $role = Duration::find($id);
            $title_en = $role->title;
            $status = $role->status;
            $title_ar = $role->title_ar;
            $title_ban = $role->title_ban;
        }
        return view('admin.durations.create', compact('mode', 'page_heading', 'id', 'status', 'title_en', 'title_ar','title_ban'));
    }
    ...
    public function submit(REQUEST $request)
    {
        ...
        $o_data     = [];
        $errors     = [];
        $o_data['redirect'] = route('admin.durations.list');
        $rules = [
            'title_en' => [
                'required',
                'min:2',
        ...
        if ($validator->fails()) {
            ...
            $id         = $request->id;
            $check      = Duration::whereRaw('Lower(title) = ?', [strtolower($title_en)])->where('id', '!=', $id)->get();

            if ($check->count() > 0) {
                $status = "0";
                $message = "Duration Already Addded";
                $errors['title_en'] = 'Duration Already Added';
                if($request->json_action){
                    return response()->json([
                        'success' => true,
                        'message' => "Duration Already Added successfully",
                        'id' => '',
                        'name' => ''
                    ]);
                }
    ...
    public function change_status(REQUEST $request)
    {
        ...
        $item = Duration::where(['id' => $id])->get();

        if ($item->count() > 0) {

            Duration::where('id', '=', $id)->update(['status' => $request->status]);
            $status = "1";
    ...
    public function delete(REQUEST $request, $id)
    {
        ...
        $category_data = Duration::where(['id' => $id])->first();
