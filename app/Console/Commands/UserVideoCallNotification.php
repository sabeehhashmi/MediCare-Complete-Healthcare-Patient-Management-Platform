...
class UserVideoCallNotification extends Command
{
    protected $signature = 'video:user_call {user_id} {channel}';
    ...
    public function handle()
    {
        ...
        $order = DoctorPatientAppointment::with(['user', 'doctor','doctor.user'])
           ->where('doctor_patient_appointments.booking_id','=','#'.$channel)->get()->first();
           
          
        
         $doctor_id =$order->user->id;
        ...
        $doctor_id =$order->user->firebase_user_key;
        

        $data["Nottifications/".$doctor_id."/".$notification_id] = [
