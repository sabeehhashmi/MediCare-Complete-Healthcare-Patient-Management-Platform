...
return new class extends Migration
{
    public function up()
    {
        Schema::table('doctor_patient_appointments', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('doctor_patient_appointments', 'doctor_earning')) {
                $table->decimal('doctor_earning', 10, 2)->nullable()->after('admin_commission');
            }

            if (!Schema::hasColumn('doctor_patient_appointments', 'payment_token')) {
                $table->string('payment_token', 100)->nullable()->after('doctor_earning');
            }
            ...
            if (!Schema::hasColumn('doctor_patient_appointments', 'payment_email_sent_at')) {
                $table->timestamp('payment_email_sent_at')->nullable()->after('payment_completed_at');
            }

            if (!Schema::hasColumn('doctor_patient_appointments', 'urgent_notified_at')) {
                $table->timestamp('urgent_notified_at')->nullable()->after('payment_email_sent_at');
            }
        });
    }
