...
<head>
        ...
        <meta charset="utf-8" />
        <title>Hospital Login | {{config('global.site_name')}}</title>
                        ...
                        <div class="card overflow-hidden">
                            <div class="card-body p-4"> 
                                ...
                                <div class="text-center mt-2">
                                    <h5>Welcome Back !</h5>
                                    <p class="text-black">Sign in to continue to Hospital Panel.</p>
                                </div>
                                <div class="p-2 mt-4">
                                    <form class="form-login" action="{{ url('hospital/check_login') }}" method="POST">
                                       @csrf
                                        <div class="mb-3">
                                        ...
                                        <div class="mt-3 text-center">
                                            <p class="mb-0">Don’t have an account? <a href="{{ url('hospital/register') }}" class="fw-medium text-primary"> Sign Up </a> </p>
                                        </div>
                                    </form>
                                </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="forgot-password-form" method="POST" action="{{url('forgot-password')}}">
                    @csrf
                    <input type="hidden" name="role" value="{{HOSPITAL_ROLE}}">
                    <div class="mb-3">
                        <label class="form-label" for="username">Email</label>
                        <div class="position-relative input-custom-icon">
                    ...
                    <div class="mt-3 text-center">
                    <p class="mb-0">Don’t have an account? <a href="{{ url('hospital/register') }}" class="fw-medium text-primary"> Sign Up </a> </p>                    </div>
                </form>
            </div>
        </div>
        <!-- /.modal-content -->
...
<!-- /.modal -->
    ...
    $.ajax({
        type:'POST',
        url: "{{ route('hospital.check_login') }}",
        data:{
...
$("#loginOtpForm").submit(function(e) {
    ...
    $.ajax({
        type: 'POST',
        url: "{{ route('hospital.verify.login.otp') }}",
        data: {
            _token: $('input[name=_token]').val(),
