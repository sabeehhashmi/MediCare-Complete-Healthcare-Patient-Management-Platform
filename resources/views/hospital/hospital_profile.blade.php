        ...
        <div class="w-100 user-chat mt-4 mt-sm-0 ms-lg-3">
            <div class="card">
                <div class="text-center bg-light rounded px-4 py-3">
                    ...
                    <h5 class="font-size-16 mb-1 mt-3"><a href="#" class="text-reset">{{$name}} Hospital </a></h5>
                </div>
                    ...
                    <div class="table-responsive mt-3 pb-3">
                        <table class="table align-middle table-sm table-wrap table-borderless table-centered mb-0">
                            <tbody>
                                ...
                                {{-- <tr>
                                    <td class="text-muted">Direct Call for Appointment Number :</td>
                                    <th class="fw-bold">+{{$hospital->appointment_dial_code}} {{$hospital->appointment_phone}}</th>
                                </tr> --}}
                                <!-- end tr -->
            ...
            <div class="card mt-4">
                <div class="card-body">
                    ...
                    <h5 class="font-size-24 mb-3">Hospital Profile</h5>
                    <div class="mt-3">
...
<script>
    $(document).ready(function() {
        $('#imageUpload').on('change', function() {
            ...
            $.ajax({
                url: '{{ route("hospital.save_profile_image") }}',
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
...
$(document).on('click', '#deleteAccountBtn', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                url: "{{ route('hospital.delete.account') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}"
                },
