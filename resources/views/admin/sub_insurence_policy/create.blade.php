...
@section('content')
<div class="card mb-5">
    <form id="admin_form" method="post" action="{{route('admin.sub_insurence_policy.submit')}}" enctype="multipart/form-data" class="custom-form">
        <div class="card-body">
            <div class="row">
                    ...
                    <div class="col-lg-6 mb-4">
                        ...
                        <select name="insurence_id" class="form-control jqv-inuput" role="select2" data-jqv-required="true">
                            <option value="">Select</option>
                            @foreach($insurence_list as $item)
                            <option {{($item->id==$insurence_id)?'selected':''}} value="{{$item->id}}">{{$item->title}}</option>
                            @endforeach
                        </select>
                    </div>
