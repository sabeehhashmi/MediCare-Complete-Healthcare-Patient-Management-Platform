...
@section('content')
<div class="card mb-5">
    <form id="admin_form" method="post" action="{{route('admin.refferal_doctors.submit')}}" enctype="multipart/form-data" class="custom-form">
        <div class="card-body">
            <div class="row">
                    @csrf()
                    ...
                    <div class="col-lg-6 mb-4">
                        <label class="form-label" for="bs-validation-insurence">Department<span class="text-danger">*</span> </label>
                        <select name="department_id" class="form-control jqv-inuput" role="select2" data-jqv-required="true">
                            <option value="">Select</option>
                            @foreach($departments as $item)
                            <option {{($item->id==$department)?'selected':''}} value="{{$item->id}}">{{$item->title}}</option>
                            @endforeach
                        </select>
                    </div>
