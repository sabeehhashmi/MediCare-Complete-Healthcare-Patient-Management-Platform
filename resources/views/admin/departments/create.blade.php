...
@section('content')
<div class="card mb-5">
    <form id="admin_form" method="post" action="{{route('admin.departments.submit')}}" enctype="multipart/form-data" class="custom-form">
        <div class="card-body">
            <div class="row">
                    @csrf()
                        ...
                        <div class="position-relative select-custom-icon">
                            ...
                            <option  value=""></option>
                                @foreach($referrals as $referral)
                                    <option {{($referral->id == ($referral_id ?? null)) ? "selected" : ""}}  value="{{$referral->id}}">{{$referral->title}}</option>
                                @endforeach
