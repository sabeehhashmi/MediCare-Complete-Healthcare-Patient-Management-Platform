                ...
                <div class="modal-body">
                    ...
                    @csrf
                        ...
                        <div class="col-12 mb-3">
                            ...
                            <div class="col-12 mb-3">
                                <label class="form-label" for="">Reason of Reschedule <span class="text-danger">*</span></label>
                                <div class="position-relative">
                                    <textarea class="form-control" id="reason_reschedule" name="reason_reschedule"  value ="" rows="2" required></textarea>
                                </div>
                            </div>
...
$(document).ready(function () {
    ...
    var table = $('#table_list').DataTable({
        ...
        ajax: {
            'type': 'POST',
            'url': '{{ route("admin.appointments.loadData") }}',
            'data': function(d) {
        ...
        columns: [
            ...
            { data: 'booking_time_slot', name: 'booking_time_slot'},
             { data: 'call_live_status', name: 'call_live_status'},
    ...
    function updateSlots(slots, selectedDate, selectedTime = null) {
        ...
        if (slots) {
            ...
            $('#reschedule-modal #confirm_reschedule_appointment').on('click', function(e) {
                ...
                var reason = $('#reason_reschedule').val();
                
                // Check if reason is empty
                if (!reason || reason.trim() === '') {
                    App.alert('Please provide a reason for rescheduling', 'Error!', 'error');
                    $('#reason_reschedule').addClass('is-invalid');
                    return false;
                }
