            ...
            <div class="row">
              <div class="col-md-3">
                <div class="form-group">
                  <input class="form-control" type="text" name="search_key" value="{{$_GET['search_key']??''}}" placeholder="Customer Name">
                </div>
              </div>
              <div class="col-md-3">
                <div class="form-group">
              ...
              <div class="from-group">
                ...
                <a href="{{route('admin.list_customers')}}" class="btn btn-success">Clear</a>
              </div>
            </div>
          </form>
        </div>
      ...
      <table class="table table-condensed table-striped" id="example2">
        <thead>
          <tr>
            ...
            <th data-colname="name">Customer Name </th>
            <th data-colname="email">Email</th>
            <th data-colname="phone">Phone</th>
            <th data-colname="user_code">User Code</th>
            <th data-colname="refferal_code">Refferel code</th>
            <th>Status</th>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{ $role->name }}</td>
            <td>{{ $role->email }}</td>
            <td>+{{ $role->dial_code }} {{ $role->phone }}</td>
            <td>{{ $role->user_code }}</td>
            <td>{{ $role->refferal_code }}</td>
            ...
            <td>
              <div class="dropdown">
                ...
                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                  ...
                  @endif -->
                  @if (get_user_permission('customers', 'u'))
                  <a class="dropdown-item" href="{{ route('admin.view_user_referls', ['id' => $role->id]) }}">
                    <i class="flaticon-list"></i> View Referrals
                  </a>
                  @endif

                  @if (get_user_permission('customers', 'd'))
                  <a class="dropdown-item" data-role="unlink"
                    data-message="Do you want to remove the customer? "
                    href="{{ route('admin.delete_customer', ['id' => $role->id]) }}">
