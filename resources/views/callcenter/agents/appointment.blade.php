    ...
    <div class="card-body">
        ...
        <div class="col-sm">
                    <div class="mt-3 mt-md-0 mb-3 d-flex gap-2">
                        ...
                        <button type="button" class="btn btn-sm btn-icon btn-dark text-white" id="reset-filters" ><i class="mdi mdi-file-excel"></i></button>
...
<script>
    $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res) {
                            ...
                            $.each(res, function (index, data) {
                                if(data.user)
                                $('#doctorSelect').append('<option value="' + data.user_id+'" data-id="'+data.id+'">' + data.user?.name + '</option>');
                            });
                            // $('#doctorSelect').val(selectedId).trigger('change');
                            $('#doctorSelect').select2({ dropdownParent: "#appointment-modal" }) // Reinitialize select2
                        }
                    },
                    error: function (xhr, status, error) {
                        console.error('Error fetching Members:', error);
        ...
        $('#appointment-modal #booking_date_appointment, #appointment-modal #doctorSelect').on("change", function () {
            checkAvailibility2($('#appointment-modal #doctorSelect').val(), $('#appointment-modal #booking_date_appointment').val());
        });
...
$(document).ready(function () {
   ...
   var table =  $('#table_list').DataTable({
        ...
        ajax: {
            'type':'POST',
            'url' : '{{ route("callcenter.agents.appointmentLoadData") }}',
            data: function(d) {
                d.agent_id = value
                d._token = '{{ csrf_token() }}';
...
document.getElementById("exampleModalLabel").innerText = "Reschedule Booking- " + param1;
    ...
    $(document).ready(function() {
        ...
        $("#PatientSelect, #doctorSelect, #ClinicSelect, #DepartmentSelect").select2({ dropdownParent: "#appointment-modal" });
...
<script>
    ...
    //                         $('#DepartmentSelect').append('<option value="' + department.id+'">' + department.title + '</option>');
    //                     });
        ...
        if (hospital_id) {
            $.ajax({
                ...
                success: function (res) {
                    if (res) {
                        $('#DepartmentSelect').empty();
                        $('#DepartmentSelect').append('<option value="">Select Departments</option>');
                        $.each(res, function (index, department) {
                            $('#DepartmentSelect').append('<option value="' + department.id + '">' + department.title + '</option>');
                        });
                        // $('#DepartmentSelect').val(selectedEmirateId).trigger('change');
