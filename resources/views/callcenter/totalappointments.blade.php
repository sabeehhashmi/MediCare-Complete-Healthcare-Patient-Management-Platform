...
<div class="card">
    <div class="card-body">
        <div class="position-relative">
            ...
            <form action="{{ route('callcenter.totalappointments') }}" method="GET" id="search-form">
                <div class="row align-items-end">
                    ...
                    <div class="col-sm">
                        ...
                        <div class="d-flex mt-3 mb-3">
                            ...
                            <a href="{{route('callcenter.totalappointments')}}" type="button"  class=" btn btn-info waves-effect waves-light">Refresh</a>
                            <a href="{{ route('callcenter.appointments.export') }}" id="export" class="btn btn-primary"><i class="mdi mdi-file-excel"></i> Export</a>
                        </div>
                        <!-- </div> -->
                    </div>
                </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="form-check form-switch">
                                        <input type="checkbox" class="form-check-input toggle-urgent" 
                                            data-id="{{ $appointment->id }}" 
                                            data-urgent="{{ $appointment->is_urgent ? 1 : 0 }}"
                                            {{ $appointment->is_urgent ? 'checked' : '' }}>
                                    </div>
                                </td>
                                ...
                                <td>
                                    <div class="dropdown mt-4 mt-sm-0">
                                        ...
                                        <div class="dropdown-menu">
                                                <a class="dropdown-item cancel-link" href="{{route('callcenter.appointmentdetail',['id'=>$appointment->id])}}">View Appointment</a>
                                               ...
                                               <a class="dropdown-item" data-role="unlink"
                                                    data-message="Do You want to make this urgent"
                                                    href="{{ route('callcenter.patient_appointment_urgent', ['id' => $appointment->id])  }}">
                                                    Make Urgent
                                                </a>
                                                @endif
                                        </div>
...
function reformatDate(dateStr) {
    ...
    $(document).ready(function() {
         ...
         $('body').on('click', '[data-role="unlink"]', function(e) {
            ...
            App.confirm('Confirm Urgent', msg, function() {
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        if (res.status == 1) {
                            App.alert(res.message || 'Made Urgent successfully', 'Success!');
                            setTimeout(function() {
                                window.location.reload(); // Refresh page after successful delete
                            }, 1500);
                        } else {
                            App.alert(res.message || 'Unable to make urgent this record.', 'Failed!');
                        }
                    },
    ...
    function loadDoctorsDepartmentMDL(departmentId){
        var hospitalId = $('#appointment-modal #hospitalSelct').val()
        if(departmentId){
            $('#doctorSelct').html('<option value="" disabled>Loading..</option>');
            $.ajax({
                type: "GET",
                url: "{{ url('callcenter/get-department-doctors') }}/" + departmentId+'/'+hospitalId,
                success: function (res) {
                    if (res) {
                        // $('#doctorSelct').empty();
                        $('#doctorSelct').html('<option value="">Select Doctor</option>');
    ...
    function loadDepartmentsMDL(hospital_id){
        ...
        $('#appointment-modal #hospitalSelct').on("change", function () {
        ...
        $('#appointment-modal #DepartmentSelct').on("change", function () {
...
$(document).on('change', '.toggle-urgent', function() {
    var checkbox = $(this);
    var appointmentId = checkbox.data('id');
    var isUrgent = checkbox.prop('checked') ? 1 : 0;
    
    $.ajax({
        url: "{{ route('callcenter.appointments.toggleUrgent') }}",
        type: "POST",
        data: {
            _token: "{{ csrf_token() }}",
        ...
        success: function(response) {
            if (response.status == 1) {
                App.alert(response.message, 'Success!', 'success');
            } else {
                checkbox.prop('checked', !isUrgent);
                App.alert(response.message, 'Error!', 'error');
            }
        },
        error: function() {
            checkbox.prop('checked', !isUrgent);
            App.alert('Network error', 'Error!', 'error');
        }
    });
});
