{{-- resources/views/callcenter/chat/index.blade.php --}}
...
$(document).ready(function() {
    ...
    async function init() {
        try {
            let res = await fetch('{{ route("callcenter.chat.init") }}', {
                method: 'POST',
            ...
            refreshInterval = setInterval(() => {
                if (isReady) refreshConversationList();
            }, 3000);
    ...
    function setupMessageListener() {
        ...
        messageListenerId = "callcenter_chat_" + Date.now();
    ...
    // FIXED: Handle new message - shows instantly for open chat
    async function handleNewMessage(message) {
        ...
        const isCurrentChat = (currentReceiver === senderUid);
        
        // Get message text correctly - CometChat returns getText() method
        let messageText = message.text || (message.getText ? message.getText() : '');
        let messageType = message.type || (message.getType ? message.getType() : 'text');
        let url = null;
        let name = null;
        ...
        if (messageType === 'image') {
            messageText = '📷 Image';
            url = message.data?.url || (message.getData ? message.getData().url : '');
            ...
            messageText = '📎 File';
            url = message.data?.url || (message.getData ? message.getData().url : '');
            name = message.data?.name || (message.getData ? message.getData().name : 'File');
        }
        
        // If this is the currently open chat - add message to UI immediately
        if (isCurrentChat) {
            console.log('Adding message to current chat UI:', messageText);
            ...
            try {
                await cometChat.markAsRead(senderUid, cometChat.RECEIVER_TYPE.USER);
            } catch(e) {}
            ...
            // Not current chat - just show notification and update list
    ...
    async function loadUsers() {
        try {
            let res = await fetch('{{ route("callcenter.chat.conversations") }}', {
            ...
            html += `
                <div class="conversation-item ${active}" data-uid="${chat.receiver_id}" data-name="${escapeHtml(chat.name)}" data-avatar="${chat.avatar || ''}">
                    <div class="conversation-avatar">
                        <img src="${chat.avatar || '{{ asset("callcenter/assets/images/default-avatar.jpg") }}'}" onerror="this.src='{{ asset("callcenter/assets/images/default-avatar.jpg") }}'">
                    ...
                    <div class="conversation-info">
                        ...
                        <div class="conversation-type">${escapeHtml(chat.type || 'Contact')}</div>
    ...
    async function openChat(uid, name, avatar) {
        ...
        $('#chatAvatar').attr('src', avatar || '{{ asset("callcenter/assets/images/default-avatar.jpg") }}');
        $('#inputArea').show();
    ...
    async function sendMessage() {
        ...
        try {
            ...
            await fetch('{{ route("callcenter.chat.notify") }}', {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Content-Type': 'application/json' },
