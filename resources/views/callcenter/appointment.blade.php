...
<div class="card">
    <h5 class="card-header card-header d-flex justify-content-between">{{$page_heading}}
        @php
            $disabled = false;
        ...
        @else
            <button type="button"  class="btn btn-primary m-0 " data-bs-toggle="modal" data-bs-target="#appointment-modal-doctor">Make an Appointment</button>
        ...
        <div class="position-relative">
            <!-- <div class="d-flex gap-2 justify-content-between mb-3">
                <div class="btn-group toggle-btn-g-tabs-cale" role="group" aria-label="Basic checkbox toggle button group">
                    ...
                    <label class="btn btn-outline-primary" for="btncheck4"><img class="icn" src="{{ asset('') }}callcenter/assets/images/table-view.png" alt="" /></label>

                    <input type="radio" class="btn-check" name="scs" id="btncheck5" autocomplete="off" />
                    <label class="btn btn-outline-primary" for="btncheck5"><img class="icn" src="{{ asset('') }}callcenter/assets/images/calender-icon.png" alt="" /></label>
                </div>
    ...
    <form action="{{ route('callcenter.appointments', $param) }}" method="GET" id="search-form">
    @if($doctor)
    <input type="hidden" name="doctor_id" value="{{$doctor->id ?? null}}">
    @endif
        @if($clinic)
        <input type="hidden" name="clinic_id" value="{{$clinic->id ?? null}}">
        @endif
        @if($hospital)
        <input type="hidden" name="hospital_id" value="{{$hospital->id ?? null}}">
        @endif
        ...
        <div class="col-sm">
            ...
            <div class="d-flex mt-3 mb-3">
                <!-- <div class="mt-3 mt-md-0 mb-3 me-3">
                    ...
                    <button type="button" id="clear-search" class=" btn btn-info waves-effect waves-light">Refresh</button>
                </div> -->
                <button type="submit" id="" class="btn btn-primary">Search</button>
                <a href="{{route('callcenter.appointments', $param)}}" type="button"  class=" btn btn-info waves-effect waves-light">Refresh</a>
                <a href="{{ route('callcenter.appointments.export', $param) }}" id="export" class="btn btn-primary"><i class="mdi mdi-file-excel"></i> Export</a>
            </div>
            </div>
        </div>
                                ...
                                <td>
                                    <div class="dropdown mt-4 mt-sm-0">
                                        ...
                                        <div class="dropdown-menu">
                                                <a class="dropdown-item" href="{{route('callcenter.appointmentdetail',['id'=>$appointment->id])}}">View Appointment</a>
...
<script>
    ...
    function updateExportUrl() {
        ...
        let exportUrl = new URL(exportElement.attr('href') || "{{ route('callcenter.appointments.export') }}");
    ...
    $(document).ready(function() {
        ...
        function loadDoctorsDepartment(departmentId){
            let hospitalId = $('#hospital_id').val();
            if(!hospitalId){
                hospitalId = $('#clinic_id').val();
            }
            if(departmentId){
                $.ajax({
                    type: "GET",
                    url: "{{ url('agent/get-department-doctors') }}/" + departmentId+'/'+hospitalId,
                    success: function (res) {
                        if (res) {
                            $('#doctorSelct').empty();
                            $('#doctorSelct').append('<option value="">Select Doctor</option>');
        ...
        $('#appointment-modal-doctor #booking_date_appointment, #appointment-modal-doctor #doctorSelct1').on("change", function () {
            checkAvailibility2($("#doctorSelct1").val(), $('#appointment-modal-doctor #booking_date_appointment').val());
        });
...
<script>
    ...
    function updateSlots2(slots, selectedDate) {
        ...
        if (slots && Array.isArray(slots)) {
            $('#appointment-modal-doctor .availiblity').each(function() {
                var slot = $(this).val();
    ...
    $('#appointment-modal-doctor #book_appointment_dr').on('click', function(e) {
        e.preventDefault();

        var $form = $('#book_appointment_form_dr');
        var formData = new FormData($form[0]);
        ...
        let i = 0;
        App.setJQueryValidationRules('#book_appointment_form_dr');
