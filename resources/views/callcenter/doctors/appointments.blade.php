...
<div class="card mb-5">
    ...
    <div class="col-lg-12 mb-5">
            <a  href=" {{route('callcenter.doctors')}}"  class="btn btn-primary float-end">Back</a>
            </div>
...
$(document).ready(function () {
  ...
  var table =  $('#table_list').DataTable({
        ...
        ajax: {
            'type':'POST',
            'url' : '{{ route("callcenter.doctors.appointmentLoadData") }}',
            data: function(d) {
                d.doctor_id = value
                d._token = '{{ csrf_token() }}';
...
$("#cancelAppointment").click(function(e){
     ...
     let data = new FormData(form);
      ...
      $.ajax({
        url: "{{ route('callcenter.patient_appointment_cancel') }}",
        type: "POST",
        data : data,
        dataType:"JSON",
        processData : false,
...
$("#confirmReschedule").click(function(e){
     ...
     let data = new FormData(form);
      ...
      $.ajax({
        url: "{{ route('callcenter.patienttAppointmentRescheduled') }}",
        type: "POST",
        data : data,
        dataType:"JSON",
        processData : false,
