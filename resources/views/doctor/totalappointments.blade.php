...
<div class="card">
    <div class="card-body">
        <div class="position-relative">
            ...
            <div class="btn-group toggle-btn-g-tabs-cale" role="group" aria-label="Basic checkbox toggle button group">
                            ...
                            <label class="btn btn-outline-primary" for="btncheck4"><img class="icn" src="{{ asset('') }}doctor/assets/images/table-view.png" alt="" /></label>

                            <input type="radio" class="btn-check" name="scs" id="btncheck5" autocomplete="off" />
                            <label class="btn btn-outline-primary" for="btncheck5"><img class="icn" src="{{ asset('') }}doctor/assets/images/calender-icon.png" alt="" /></label>
                        </div>
    ...
    <form action="{{ route('doctor.totalappointments') }}" method="GET" id="search-form">
    <div class="row align-items-end">
        ...
        <div class="col-lg-3 col-md-6 mb-3">
            ...
            <div class="position-relative select-custom-icon">
                <select name="booking_status" id="booking_status" class="select2-single" data-placeholder="Select Status">
                    <option></option>
        ...
        <option value="{{ $type->name }}" {{ old('booking_status', request('booking_type')) == $type->name ? 'selected' : '' }} >
            {{ $type->name }}
        </option>
    @endforeach
                </select>
        ...
        <div class="col-sm">
            <div class="d-flex justify-content-between">
                <div class="d-flex mt-3 mb-3">
                    ...
                    <a href="{{route('doctor.totalappointments')}}" type="button"  class=" btn btn-info waves-effect waves-light">Refresh</a>
                    <a href="{{ route('doctor.appointments.export') }}" id="export" class="btn btn-primary"><i class="mdi mdi-file-excel"></i> Export</a>
                </div>
            </div>
        </div>
    </div>
                               ...
                               <td>
                                @php
                                    if ($appointment->status_history->count()) {
                                        ...
                                        echo ($appointment->created_by_user)? $appointment->created_by_user->name:'';
                                    }
                                @endphp
                               </td>
                                ...
                                <td>
                                    <div class="dropdown mt-4 mt-sm-0">
                                        ...
                                        <div class="dropdown-menu">
                                                <a class="dropdown-item cancel-link" href="{{route('doctor.appointmentdetail',['id'=>$appointment->id])}}">View Appointment</a>
    ...
    $(document).ready(function() {
        ...
        function loadMember(parentId){
            if (parentId) {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ url('doctor/get-members') }}/" + parentId,
                ...
                // $('#memberSelct').empty();
                $('#memberSelct').htnl('<option value=""></option>');
            }
        }
        ...
        function loadDepartments(parentId){
            if (parentId) {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ url('doctor/get-hospital-departments') }}/" + parentId,
                    success: function (res) {
                        if (res) {
                            // $('#department').empty();
        ...
        function loadDoctorsHospital(hospitalId, departmentId){
            if (hospitalId) {
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ url('doctor/get-hospital-doctors') }}/" + hospitalId,
                    success: function (res) {
                        if (res) {
                            // $('#doctorSelct').empty();
        ...
        function loadDoctorsDepartment(departmentId){
            if(departmentId){
                ...
                $.ajax({
                    type: "GET",
                    url: "{{ url('doctor/get-department-doctors') }}/" + departmentId,
                    success: function (res) {
                        if (res) {
                            // $('#doctorSelct').empty();
                            $('#doctorSelct').html('<option value="">Select Doctor</option>');
        ...
        $('#appointment-modal #booking_date_appointment').on("change", function () {
            checkAvailibility2($('#appointment-modal #booking_date_appointment').val());
        });
