...
<head>
        ...
        <meta charset="utf-8" />
        <title>Doctor Panel | {{config('global.site_name')}}</title>
        ...
        const dbRef = query(ref(database, 'Doctor/'+doctor_user_Id), limitToLast(5));
        ...
        onValue(dbRef, (snapshot) => {
            ...
            dataArray.forEach((item) => {
                ...
                const notificationItem = document.createElement('a');
                 if(item.type === 'call'){
                    notificationItem.href = `{{url('video-call')}}/${item.channel_name}`;
                } else if (item.notificationType === 'bulk_broadcast') {
                    notificationItem.href = `{{route('doctor.notifications')}}`;
                } else if (item.notificationType === 'chat_message') {
                    notificationItem.href = `{{ url('doctor/chat') }}`;
                } else {
                    notificationItem.href = `{{url('doctor/appointmentdetail')}}/${item.order_id}`;
                }
                ...
                notificationItem.innerHTML = `
                    <div class="d-flex">
                        <div class="flex-shrink-0 me-3">
                            <img src="${item.imageURL}" onerror="this.onerror=null; this.src='{{ URL::asset('admin-assets/assets/images/doctor_placeholder.jpg') }}'"
            ...
            <div class="d-flex">
                ...
                <div class="navbar-brand-box">
                    <a href="{{url('doctor/dashboard')}}" class="logo logo-dark">
                        <span class="logo-sm">
                            <img src="{{ asset('') }}doctor/assets/images/logo-mednero-sm.png" alt="" height="26">
                        </span>
                        <span class="logo-lg">
                            <img src="{{ asset('') }}doctor/assets/images/logo-mednero.png" alt="" height="44">
                        </span>
                    </a>
            ...
            <div class="d-flex">
                ...
                <div class="dropdown d-inline-block">
                        <button type="button" class="btn header-item user text-start d-flex align-items-center" id="page-header-user-dropdown"
                            ...
                            alt="Header Avatar">
                            <span class="d-none d-xl-inline-block ms-2 fw-medium font-size-15 text-primary">DR {{ Auth::user()->name }}</span>
                        </button>
                ...
                <div class="d-flex">
                    ...
                    <div class="dropdown d-inline-block">
                        ...
                        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end p-0"
                            ...
                            <div class="p-2 border-top d-grid">
                                <a class="btn btn-sm btn-link font-size-14 btn-block text-center h-auto" href="{{route('doctor.notifications')}}">
                                    <i class="uil-arrow-circle-right me-1"></i> <span>View More..</span>
                                </a>
                            </div>
                        </div>
                    ...
                    @php
                        ...
                        $qualifications = DB::table('doctor_qualifications')
                            ...
                            ->where('doctor_qualifications.doctor_id', $doctorId)
                            ->pluck('qualifications.title')
                            ->toArray();
                    @endphp
                    <div class="dropdown d-inline-block">
                        <button type="button" class="btn header-item user text-start d-flex align-items-center" id="page-header-user-dropdown"
                            ...
                            alt="Header Avatar">
                            <span class="d-none d-xl-inline-block ms-2 fw-medium font-size-15 text-primary">DR {{ Auth::user()->first_name }} {{ Auth::user()->last_name }}</span>
                        </button>
                        <div class="dropdown-menu dropdown-menu-end pt-0">
                            <div class="p-3 border-bottom">
                                <!-- <h6 class="mb-0 text-black">DR {{ Auth::user()->first_name }} {{ Auth::user()->last_name }} @if(!empty($qualifications)) {{ implode(', ', $qualifications) }}@endif</h6> -->
                                <h6 class="mb-0 text-black">DR {{ Auth::user()->first_name }} {{ Auth::user()->last_name }}</h6>
                                <p class="mb-0 font-size-11 text-muted">{{$user->email}}</p>
                            </div>
                            <a class="dropdown-item" href="{{ url('doctor/get_profile') }}"><i class="bx bx-user-circle text-muted font-size-16 align-middle me-2"></i> <span class="align-middle">Profile</span></a>
                            <a class="dropdown-item" href="{{ url('doctor/change_password') }}"><i class="mdi mdi-key-outline text-muted font-size-16 align-middle me-2"></i> <span class="align-middle">Reset Password</span></a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item" href="{{ url('doctor/logout') }}"><i class="mdi mdi-logout text-muted font-size-16 align-middle me-2"></i> <span class="align-middle">Logout</span></a>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <nav class="navbar navbar-light navbar-expand-lg topnav-menu">
                        ...
                        <div class="collapse navbar-collapse" id="topnav-menu-content">
                            <!-- <ul class="navbar-nav">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link" href="{{ route('doctor.totalappointments') }}">
                                ...
                                <li class="nav-item dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="topnav-dashboard">
                                        <a href="{{ url('doctor/availability') }}"  class="dropdown-item" data-key="t-ecommerce">Availability</a>
                                        <a href="{{ url('doctor/holiday') }}"  class="dropdown-item" data-key="t-ecommerce">Holiday</a>
                                        <a href="{{ url('doctor/temporaryUnavailable') }}"  class="dropdown-item" data-key="t-sales">Temporary Unavailable</a>
                                    </div>
                                </li>

                                <li class="nav-item">
                                    <a class="nav-link" href="{{ url('doctor/instantAppointment') }}">
                                ...
                                <li class="nav-item">
                                    <a class="nav-link" href="{{ url('doctor/get_profile') }}">
                            ...
                            <ul class="navbar-nav">
                                <?php $patterns = array('doctor\/dashboard');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{route('doctor.dashboard')}}">
                                        <i class="bx bx-home-alt icon nav-icon"></i>
                                        <span data-key="t-dashboards">Dashboard</span>
                                    </a>
                                </li>
                                <?php $patterns = array('doctor\/totalappointments', 'doctor\/appointmentdetail');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{route('doctor.totalappointments')}}">
                                        <i class="bx bx-calendar-event icon nav-icon"></i>
                                        <span data-key="t-dashboards">Total Appointments</span>
                                    </a>
                                </li>
                                <?php $patterns = array('doctor\/availability', 'doctor\/holiday', 'doctor\/temporaryunavailable');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item dropdown">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}} dropdown-toggle arrow-none" href="#" id="topnav-dashboard" role="button"
                                        data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                        <i class="bx bx bx-time-five nav-icon"></i>
                                        <span data-key="t-dashboards">Schedule Appointment Slots</span> <div class="arrow-down"></div>
                                    </a>
                                    <?php $pattern1 = array('doctor\/availability');
                                    $pattern2 = array('doctor\/holiday');
                                    $pattern3 = array('doctor\/temporaryunavailable');
                                      $patterns_flattened1 = implode('|', $pattern1);
                                      $patterns_flattened2 = implode('|', $pattern2);
                                      $patterns_flattened3 = implode('|', $pattern3);
                                ?>
                                    <div class="dropdown-menu" aria-labelledby="topnav-dashboard">
                                        <a href="{{ url('doctor/availability') }}"  class="dropdown-item {{preg_match('/'.$patterns_flattened1.'/', $CurrentUrl) ? 'active' : null}}" data-key="t-ecommerce">Availability</a>
                                        <a href="{{ url('doctor/holiday') }}"  class="dropdown-item {{preg_match('/'.$patterns_flattened2.'/', $CurrentUrl) ? 'active' : null}}" data-key="t-ecommerce">Holiday</a>
                                        <a href="{{ url('doctor/temporaryunavailable') }}"  class="dropdown-item {{preg_match('/'.$patterns_flattened3.'/', $CurrentUrl) ? 'active' : null}}" data-key="t-sales">Temporary Unavailable</a>
                                    </div>
                                </li>
                                <?php $patterns = array('doctor\/instantAppointment');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <!--<li class="nav-item">-->
                                <!--    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{ url('doctor/instantAppointment') }}">-->
                                <!--        <i class="bx bx-calendar-check icon nav-icon"></i>-->
                                <!--        <span data-key="t-dashboards">Schedule Instant Appointment Date</span>-->
                                <!--    </a>-->
                                <!--</li>-->
                                ...
                                <?php $patterns = array('doctor\/reports');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <!--<li class="nav-item">-->
                                <!--    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{route('doctor.reports')}}">-->
                                <!--        <i class="bx bx-file icon nav-icon"></i>-->
                                <!--        <span data-key="t-dashboards">Reports</span>-->
                                <!--    </a>-->
                                <!--</li>-->
                                <li class="nav-item dropdown">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}} dropdown-toggle arrow-none" href="#" id="topnav-report" role="button"
                                        data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                        <i class="bx bx-file icon nav-icon"></i>
                                        <span data-key="t-dashboards">Reports</span> <div class="arrow-down"></div>
                                    </a>
                                    <div class="dropdown-menu" aria-labelledby="topnav-report">
                                        <a href="{{route('doctor.reports')}}"  class="dropdown-item {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" data-key="t-ecommerce">Appointment Reports</a>
                                        <a href="{{route('doctor.reports')}}"  class="dropdown-item {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" data-key="t-ecommerce">Patinets Reports</a>
                                        </div>
                                </li>

                                <?php $patterns = array('doctor\/earnings', 'doctor\/withdrawal');
                                    $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item dropdown">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}} dropdown-toggle arrow-none" href="#" id="topnav-earnings" role="button"
                                        data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                        <i class="bx bx-wallet icon nav-icon"></i>
                                        <span data-key="t-earnings">My Earnings</span> 
                                        <div class="arrow-down"></div>
                                    </a>
                                    <div class="dropdown-menu" aria-labelledby="topnav-earnings">
                                        <a href="{{ route('doctor.earnings.index') }}" class="dropdown-item" data-key="t-earnings-dashboard">
                                            Earnings Dashboard
                                        </a>
                                        <a href="{{ route('doctor.withdrawal.history') }}" class="dropdown-item" data-key="t-withdrawal-history">
                                            Withdrawal History
                                        </a>
                                    </div>
                                </li>
                                ...
                                <?php $patterns = array('doctor\/edit_profile','doctor\/get_profile');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{ url('doctor/get_profile') }}">
                                        <i class="bx bx-user-circle icon nav-icon"></i>
                                        <span data-key="t-dashboards">Profile</span>
                                    </a>
                                </li>

                                <?php $patterns = array('doctor\/chat');
                                    $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{ route('doctor.chat.index') }}">
