...
<head>
        ...
        <meta charset="utf-8" />
        <title>Clinic Panel | {{config('global.site_name')}}</title>
    ...
    <script type="module">
        ...
        function showToast(item) {
            ...
            if (item.notificationType === 'bulk_broadcast') {
                toast.href = `{{route('clinic.notifications')}}`;
            } else if (item.notificationType === 'chat_message') {
                toast.href = `{{ url('clinic/chat') }}`;
            } else {
                toast.href = `{{url('clinic/appointmentdetail')}}/${item.order_id}`;
            }
        ...
        onValue(dbRef, (snapshot) => {
            ...
            dataArray.forEach((item) => {
                ...
                if (item.notificationType === 'bulk_broadcast') {
                    ...
                    notificationItem.href = `{{ url('clinic/chat') }}`;
                } else {
        ...
        <div class="navbar-header">
            <div class="d-flex">
                ...
                <div class="navbar-brand-box">
                    <a href="{{route('clinic.dashboard')}}" class="logo logo-dark">
                        <span class="logo-sm">
                    ...
                    <a href="{{route('clinic.dashboard')}}" class="logo logo-light">
                        <span class="logo-lg">
                            <img src="{{ asset('') }}hospital/assets/images/logo-mednero.png" alt="" height="48">
                        </span>
                        <span class="logo-sm">
            ...
            <div class="d-flex">
                ...
                <div class="dropdown d-inline-block">
                    ...
                    <div class="dropdown-menu dropdown-menu-xl dropdown-menu-end p-0"
                        ...
                        <div class="p-2 border-top d-grid">
                            <a class="btn btn-sm btn-link font-size-14 btn-block text-center h-auto" href="{{route('clinic.notifications')}}">
                ...
                <div class="dropdown d-inline-block">
                        <button type="button" class="btn header-item user text-start d-flex align-items-center" id="page-header-user-dropdown"
                            ...
                            alt="Header Avatar">
                            <span class="d-none d-xl-inline-block ms-2 fw-medium font-size-15 text-primary">{{ Auth::user()->name }} Clinic</span>
                        </button>
                        <div class="dropdown-menu dropdown-menu-end pt-0">
                            <div class="p-3 border-bottom">
                                <h6 class="mb-0 text-black">Aster Clinic</h6>
                                <p class="mb-0 font-size-11 text-muted">info@bostondentaluae.com</p>
                            </div>
                            <a class="dropdown-item" href="{{ url('clinic/get_profile') }}"><i class="bx bx-user-circle text-muted font-size-16 align-middle me-2"></i> <span class="align-middle">Profile</span></a>
                            ...
                            <a class="dropdown-item" href="{{route('clinic.logout')}}"><i class="mdi mdi-logout text-muted font-size-16 align-middle me-2"></i> <span class="align-middle">Logout</span></a>
                        </div>
                    </div>
            </div>
        </div>
            ...
            <div class="navbar-header">
                ...
                <div class="d-flex">
                    ...
                    <div class="dropdown d-inline-block">
                        ...
                        <div class="dropdown-menu dropdown-menu-end pt-0">
                            <div class="p-3 border-bottom">
                                <h6 class="mb-0 text-black">{{ Auth::user()->name }} Clinic</h6>
                                <p class="mb-0 font-size-11 text-muted">{{ Auth::user()->email }}</p>
                            </div>
                            <a class="dropdown-item" href="{{ url('clinic/get_profile') }}"><i class="bx bx-user-circle text-muted font-size-16 align-middle me-2"></i> <span class="align-middle">Profile</span></a>
                            <a class="dropdown-item" href="{{ url('clinic/change_password') }}"><i class="mdi mdi-key-outline text-muted font-size-16 align-middle me-2"></i> <span class="align-middle">Reset Password</span></a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item" href="{{route('clinic.logout')}}"><i class="mdi mdi-logout text-muted font-size-16 align-middle me-2"></i> <span class="align-middle">Logout</span></a>
                        </div>
                    </div>
                ...
                <div class="container-fluid">
                    <nav class="navbar navbar-light navbar-expand-lg topnav-menu">
                        ...
                        <div class="collapse navbar-collapse" id="topnav-menu-content">
                            <ul class="navbar-nav">
                                <?php $patterns = array('clinic\/dashboard');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{route('clinic.dashboard')}}">
                                        <i class="bx bx-home-alt icon nav-icon"></i>
                                        <span data-key="t-dashboards">Dashboard</span>
                                    </a>
                                </li>
                                <?php $patterns = array('clinic\/departments','clinic\/adddepartment','clinic\/editdepartment');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <?php $patterns = array('clinic\/doctors','clinic\/doctordetail','clinic\/addoctor',   'clinic\/appointments','clinic\/availability','clinic\/temporaryunavailable','clinic\/editdoctor','clinic\/holiday','clinic\/instantappointment');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{route('clinic.doctors')}}">
                                        <i class="fi fi-rr-user-md icon nav-icon custom-icon" style="font-size: 1.0rem;"></i>
                                        <span data-key="t-dashboards">Doctors</span>
                                    </a>
                                </li>
                                <?php $patterns = array('clinic\/hospitalAppointments');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{route('clinic.hospitalAppointments')}}">
                                        <i class="bx bx-calendar-event icon nav-icon"></i>
                                        <span data-key="t-dashboards">Total Appointments</span>
                                    </a>
                                </li>

                                <?php $patterns = array('clinic\/reviews');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{route('clinic.reviews')}}">
                                        <i class="bx bx-calendar-event icon nav-icon"></i>
                                ...
                                <li class="nav-item">
                                    <a class="nav-link" href="{{url('clinic/notifications')}}">
                                        <i class="bx bx-bell icon nav-icon"></i>
                                        <span data-key="t-dashboards">Notification</span>
                                    </a>
                                </li>
                                <?php $patterns = array('clinic\/reports');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{route('clinic.reports')}}">
                                        <i class="bx bx-file icon nav-icon"></i>
                                        <span data-key="t-dashboards">Reports</span>
                                    </a>
                                </li>
                                <?php $patterns = array('clinic\/chat');
                                $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{ route('clinic.chat.index') }}">
                                ...
                                <?php $patterns = array('clinic\/edit_profile','clinic\/get_profile');
                                      $patterns_flattened = implode('|', $patterns);
                                ?>
                                <li class="nav-item">
                                    <a class="nav-link {{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}}" href="{{ url('clinic/get_profile') }}">
