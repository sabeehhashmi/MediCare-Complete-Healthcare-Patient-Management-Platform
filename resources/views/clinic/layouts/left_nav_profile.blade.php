        ...
        <div class="chat-leftsidebar card mb-5">
            <div class="card-body" style="flex: 0;">
                <div class="text-center bg-light rounded px-4 py-3">
                    ...
                    <h5 class="font-size-16 mb-1 mt-3"><a href="#" class="text-reset">{{ Auth::user()->name }} Clinic </a></h5>
                    <p class="text-muted mb-0">{{$hospital->address ?? null}}</p>
                </div>
            </div>

            <?php $patterns = array('clinic\/get_profile');
                  $patterns_flattened = implode('|', $patterns);
            ?>
            <div class="mail-list">
                <a href="{{ url('clinic/get_profile') }}" class="{{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}} border-bottom">
                ...
                <?php $patterns = array('clinic\/edit_profile');
                  $patterns_flattened = implode('|', $patterns);
                  ?>
                <a href="{{ url('clinic/edit_profile') }}" class="{{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}} border-bottom">
                    <div class="d-flex align-items-center">
                ...
                <?php $patterns = array('clinic\/ourinsurance');
                  $patterns_flattened = implode('|', $patterns);
                  ?>
                <a href="{{ url('clinic/ourinsurance') }}" class="{{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}} border-bottom">
                    <div class="d-flex align-items-center">
                ...
                <?php $patterns = array('clinic\/change_password');
                  $patterns_flattened = implode('|', $patterns);
                  ?>
                <a href="{{ url('clinic/change_password') }}" class="{{preg_match('/'.$patterns_flattened.'/', $CurrentUrl) ? 'active' : null}} border-bottom">
                    <div class="d-flex align-items-center">
