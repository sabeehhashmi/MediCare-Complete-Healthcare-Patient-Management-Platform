...
<div class="position-relative mb-5">
    <div class="d-lg-flex">
        ...
        <div class="w-100 user-chat mt-4 mt-sm-0 ms-lg-3">
            <div class="card">
                <div class="p-4 pt-4">
                <form method="post" id="insuranceform" class="insuranceform" action="{{ url('clinic/saveinsurance') }}" enctype="multipart/form-data">
                  @csrf
...
<script>
    ...
    // When the insurance dropdown changes
    $('select[name="insurance"]').on('change', function () {
        const selectedInsuranceId = $(this).val(); // Get the selected insurance ID

        // Uncheck all checkboxes initially
        $('.checkbox-item input[type="checkbox"]').prop('checked', false);

        // Check if there are matching sub-insurances for the selected insurance
        if (selectedInsuranceId && insurancesData) {
            const matchingInsurance = insurancesData.find(
                insurance => insurance.insurance_id == selectedInsuranceId
            );

            if (matchingInsurance) {
                // Mark only the related sub-insurances as checked
                matchingInsurance.sub_insurances.forEach(subInsuranceId => {
                    $(`.checkbox-item input[value="${subInsuranceId}"]`).prop('checked', true);
                });
            }
        }
    });
