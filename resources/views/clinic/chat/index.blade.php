{{-- resources/views/clinic/chat/index.blade.php --}}
...
$(document).ready(function() {
    ...
    async function init() {
        try {
            let res = await fetch('{{ route("clinic.chat.init") }}', {
                method: 'POST',
    ...
    function setupMessageListener() {
        ...
        messageListenerId = "clinic_chat_" + Date.now();
    ...
    async function loadUsers() {
        try {
            let res = await fetch('{{ route("clinic.chat.conversations") }}', {
            ...
            html += `
                <div class="conversation-item ${active}" data-uid="${chat.receiver_id}" data-name="${escapeHtml(chat.name)}" data-avatar="${chat.avatar || ''}">
                    <div class="conversation-avatar">
                        <img src="${chat.avatar || '{{ asset("clinic/assets/images/default-avatar.jpg") }}'}" onerror="this.src='{{ asset("clinic/assets/images/default-avatar.jpg") }}'">
    ...
    async function openChat(uid, name, avatar) {
        ...
        $('#chatAvatar').attr('src', avatar || '{{ asset("clinic/assets/images/default-avatar.jpg") }}');
        $('#inputArea').show();
    ...
    async function sendMessage() {
        ...
        try {
            ...
            await fetch('{{ route("clinic.chat.notify") }}', {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Content-Type': 'application/json' },
