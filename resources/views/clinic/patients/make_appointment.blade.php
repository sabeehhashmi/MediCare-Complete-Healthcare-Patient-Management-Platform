@extends('clinic.template.layout')
@section('css')
...
<div class="card mb-5">
    <div class="p-4 pt-5">
        <form id="admin_form" method="post" action="{{route('clinic.patients.saveAppointment')}}" class="registerform" autocomplete="off">
            @csrf
            ...
            <div class="row">
                <div class="col-xs-12 col-sm-6">
                    ...
                    <select name="patient" id="patient" class="form-control jqv-inuput" role="select2">
                        ...
                        <option {{($patient_id ??  null) == $item->id ? 'selected':''}} value="{{$item->id}}">{{$item->first_name}} {{$item->last_name}}</option>
                        @endforeach
                    </select>
                </div>
                
                <div class="col-xs-12 col-sm-6">
                    <label class="form-label" for="bs-validation-insurence">Hospital </label>
                    <select name="hospital" id="hospital" class="form-control jqv-inuput" role="select2">
                        <option value=""></option>
                        @foreach($hospitals as $item)
                        <option {{($row->hospital_id ??  null) == $item->id ? 'selected':''}} value="{{$item->id}}">{{$item->name_en}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="col-xs-12 col-sm-6">
