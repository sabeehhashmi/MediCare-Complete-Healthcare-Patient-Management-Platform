...
<div class="position-relative mb-5">
    <div class="card">
        <div class="card-body">
        <form id="doctorform" method="post" action="{{route('agent.saveDoctor')}}" class="registerform" autocomplete="off">
            @csrf
                    ...
                    <div class="position-relative select-custom-icon">
                        ...
                        <option  value=""></option>
                            @foreach($hospital_name as $hospital)
                                <option {{($hospital->id == $hospital_id) ? "selected" : ""}} data-type="{{$hospital->type}}" value="{{$hospital->id}}">{{$hospital->name_en}}</option>
                            @endforeach
...
ClassicEditor.create(document.querySelector("#ckeditor-classic"))
    ...
    const input = document.querySelector("#phone");
        ...
        input.addEventListener("input", function (e) {
            ...
            e.target.value = value;            // Get the selected country's dial code
