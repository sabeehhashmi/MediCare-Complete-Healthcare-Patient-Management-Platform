...
<div class="card mb-5">
    ...
    <div class="card-header"><a href="{{route('agent.hospitals.create_appointment', $hospital_id)}}" class="btn btn-success btn-rounded waves-effect waves-light mb-2 me-2 "><i class="mdi mdi-plus me-1"></i> Book an Appointment </a></div>
    @endif
...
$(document).ready(function () {
    var table = $('#table_list').DataTable({
        ...
        ajax: {
            'type':'POST', 
            'url' : '{{ route("agent.hospitals.appointmentLoadData") }}',
            'data':{
                '_token': '{{csrf_token()}}',
    ...
    <script>
     ...
     let data = new FormData(form);
      ...
      $.ajax({
        url: "{{ route('doctors.patienttAppointmentSave') }}",
        type: "POST",
        data : data,
        dataType:"JSON",
        processData : false,
