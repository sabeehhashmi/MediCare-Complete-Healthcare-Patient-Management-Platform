...
<div class="card mb-5">
    <div class="p-4 pt-5">
        <form id="admin_form" method="post" action="{{route('agent.clinics.save')}}" class="registerform" autocomplete="off">
            @csrf
            ...
            <div class="row">
                ...
                <div class="col-12 mb-3">
                    ...
                    <div class="position-relative input-custom-icon">
                        <input type="text" class="form-control jqv-input autocomplete" data-jqv-required="true" id="txt_location" name="txt_location" value="{{old('txt_locations', $hospital->txt_location??null)}}" placeholder="673C+W8V - Dubai - United Arab Emirates" />
                        <span class="bx bx-building"></span>
                ...
                <div class="col-lg-6 mb-4">
                    ...
                    <div class="position-relative input-custom-icon">
                        <input type="email" class="form-control jqv-input" data-jqv-required="true" {{($hospital->user->email ?? null) ? 'readonly' : 'false'}} id="email" name="email" value="{{old('email', $hospital->user->email ?? null)}}" placeholder="Enter Address" />
                ...
                <div class="col-lg-6 mb-4">
                    ...
                    <div class="position-relative input-custom-icon">
                        <input type="text" class="form-control jqv-input" id="website" value="{{old('website', $hospital->website ?? null)}}" name="website" placeholder="Enter Website" />
                ...
                {{-- <div class="col-lg-6 mb-4">
                    ...
                    <div class="position-relative">
                        ...
                        <input type="number" class="form-control" id="phone1" name="direct_phone" value="{{old('direct_phone', $hospital->appointment_phone ?? null)}}" placeholder="Enter Direct Call for Appointment Number" />
                        <!-- <span class="bx bx-phone"></span> -->
                    </div>
                </div> --}}
...
$(document).ready(function() {
    ...
    function loadArea(emirateId, selectedAreaId = ''){
        if (emirateId) {
            $.ajax({
                type: "GET",
                url: "{{ url('agent/get-areas') }}/" + emirateId,
                success: function (res) {
                    if (res) {
                        $('#area_id').empty();
    ...
    function loadEmirates(countryId, selectedEmirateId = ''){
        if (countryId) {
            $.ajax({
                type: "GET",
                url: "{{ url('agent/get-emirates') }}/" + countryId,
                success: function (res) {
                    if (res) {
                        $('#emirate_id').empty();
      ...
      // check if fileArr length is greater than 0
       if (fileArr.length > 0) fileArr = [];
        ...
        $('.remove-image').on('click', function () {
            $.ajax({
                url: '{{ route('agent.hospital.delete-img') }}',
                type: 'post',
                data: {id: $(this).val()},
                headers: {
