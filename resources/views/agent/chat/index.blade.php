{{-- resources/views/agent/chat/index.blade.php --}}
...
$(document).ready(function() {
    ...
    async function init() {
        try {
            let res = await fetch('{{ route("agent.chat.init") }}', {
                method: 'POST',
    ...
    function setupMessageListener() {
        ...
        messageListenerId = "agent_chat_" + Date.now();
        ...
        cometChat.addMessageListener(messageListenerId, new cometChat.MessageListener({
            ...
            onMediaMessageReceived: message => {
                console.log('Media received:', message);
                handleNewMessage(message);
            },
    ...
    async function loadUsers() {
        try {
            let res = await fetch('{{ route("agent.chat.conversations") }}', {
            ...
            html += `
                <div class="conversation-item ${active}" data-uid="${chat.receiver_id}" data-name="${escapeHtml(chat.name)}" data-avatar="${chat.avatar || ''}">
                    <div class="conversation-avatar">
                        <img src="${chat.avatar || '{{ asset("agent/assets/images/default-avatar.jpg") }}'}" onerror="this.src='{{ asset("agent/assets/images/default-avatar.jpg") }}'">
    ...
    async function openChat(uid, name, avatar) {
        ...
        $('#chatAvatar').attr('src', avatar || '{{ asset("agent/assets/images/default-avatar.jpg") }}');
        $('#inputArea').show();
    ...
    async function sendMessage() {
        ...
        try {
            ...
            await fetch('{{ route("agent.chat.notify") }}', {
                method: 'POST',
                headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Content-Type': 'application/json' },
